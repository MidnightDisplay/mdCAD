...
typedef struct {
    ...
    bool *show_fps_debug;        // Pointer to external FPS debug window state
...
static inline void ui_visibility_init(ui_visibility_state_t* vis) {
    ...
    vis->show_fps_debug = NULL;         // Set via setter
...
static inline void ui_visibility_set_fps_debug_ptr(ui_visibility_state_t* vis, bool *fps_debug_open) {
    vis->show_fps_debug = fps_debug_open;
}
...
static inline void ui_visibility_draw(ui_visibility_state_t* vis) {
    ...
    if (vis->ecs_line_width) {
        igSliderFloat("ECS Line Width", vis->ecs_line_width, 0.001f, 0.1f, "%.3f", 0);
    }
    if (vis->ecs_point_size) {
        igSliderFloat("ECS Point Size", vis->ecs_point_size, 0.001f, 0.2f, "%.3f", 0);
    }
    ...
    if (vis->show_fps_debug) {
        igCheckbox("FPS Debug", vis->show_fps_debug);
    }
