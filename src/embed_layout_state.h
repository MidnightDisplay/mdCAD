...
static inline embed_layout_state_t embed_layout_state_make(bool embedded_mode, bool embedded_store_exists) {
...
static inline embed_layout_policy_t embed_layout_state_resolve(const embed_layout_state_t* state) {
    ...
    if (state->embedded_mode) {
        ...
        policy.manual_store_filename = embed_layout_state_embedded_store_filename();
        return policy;
    }
